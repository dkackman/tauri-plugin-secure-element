...
The crate (`tauri-plugin-secure-element`) and the npm package
(`tauri-plugin-secure-element-api`) are released together with matching version numbers;
always upgrade them in lockstep. Release history is in the repository's
[CHANGELOG](https://github.com/dkackman/tauri-plugin-secure-element/blob/main/CHANGELOG.md).

While on `0.x`, breaking changes may land in any release and are called out in the
changelog and in [Breaking changes](#breaking-changes) above.

From 1.0 onward this project follows [Semantic Versioning](https://semver.org). A major
version bump is required to break any of the following:

- **The JavaScript API** — the exported functions, their signatures, and the
  `SecureElementErrorCode` string values (removing or renaming a code is breaking;
  adding a new one is not, so `switch` statements should have a default arm).
- **The Rust API** — the public items of the crate as consumed from an app's `src-tauri`.
- **Permission identifiers** — the `secure-element:*` permissions and what `default`
  grants.
- **Key compatibility** — keys generated by any 1.x release remain listable, usable for
  signing, and deletable by every later 1.x release on the same platform.
- **Output formats** — public keys stay X9.62 uncompressed P-256 points (base64, see
  [Public Key Format](#public-key-format)), signatures stay DER-encoded ECDSA over the
  SHA-256 convention described in [Signature Format](#signature-format).

Not covered by semver: the wording of error `message` strings (only `code` values are
contractual), the exact text of OS authentication prompts, and the minimum supported
Rust version — an MSRV bump is a minor release. Raising a platform's minimum OS/SDK
version (e.g. Android `minSdk`) is treated as breaking.
