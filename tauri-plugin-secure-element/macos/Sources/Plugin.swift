...
class SecureEnclavePlugin: Plugin {
    ...
    @objc func checkSecureElementSupport(_ invoke: Invoke) throws {
        // On macOS, Secure Enclave is available on:
        // - Macs with T2 Security Chip (2018-2020 Intel Macs)
        // - Macs with Apple Silicon (M1, M2, M3, etc.)
        //
        // We check by attempting to create a test key with Secure Enclave token ID
        // On macOS, Secure Enclave IS the TEE (Trusted Execution Environment)
        ...
        if testKey != nil {
            ...
            invoke.resolve([
                ...
                "teeSupported": true, // Secure Enclave is macOS's TEE
            ])
        } else {
            // Failed to create key, Secure Enclave/TEE is not available
            // This means the Mac doesn't have T2 chip or Apple Silicon
            invoke.resolve([
                "secureElementSupported": false,
                "teeSupported": false,
            ])
