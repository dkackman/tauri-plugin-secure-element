...
class SecureEnclavePlugin: Plugin {
  ...
  @objc public func generateSecureKey(_ invoke: Invoke) throws {
  ...
  @objc public func listKeys(_ invoke: Invoke) throws {
    ...
    if status == errSecSuccess, let items = result as? [[String: Any]] {
      for item in items {
        guard let keyNameData = item[kSecAttrApplicationTag as String] as? Data,
              let keyName = String(data: keyNameData, encoding: .utf8) else {
          continue
        }
  ...
  @objc public func signWithKey(_ invoke: Invoke) throws {
  ...
  @objc public func deleteKey(_ invoke: Invoke) throws {
