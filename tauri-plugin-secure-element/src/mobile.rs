...
pub fn init<R: Runtime, C: DeserializeOwned>(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        return Ok(SecureElement {
            _phantom: std::marker::PhantomData::<fn() -> R>,
        });
    }
    #[cfg(target_os = "android")]
    {
        let handle = api.register_android_plugin("net.kackman.secureelement", "SecureKeysPlugin")?;
...
#[cfg(target_os = "macos")]
pub struct SecureElement<R: Runtime> {
    _phantom: std::marker::PhantomData<fn() -> R>,
}
...
impl<R: Runtime> SecureElement<R> {
    ...
    pub fn list_keys(&self, payload: ListKeysRequest) -> crate::Result<ListKeysResponse> {
        #[cfg(target_os = "macos")]
        {
            ...
            let list_response: ListKeysResponse = serde_json::from_str(&result_str).map_err(|e| {
                crate::Error::Io(std::io::Error::new(
                    std::io::ErrorKind::InvalidData,
                    format!("Failed to deserialize ListKeysResponse: {} (JSON was: {})", e, result_str),
                ))
            })?;
